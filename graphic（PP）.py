...
class LogConsole:
    ...
    def open_window(self) -> None:
        ...
        tk.Entry(toolbar, textvariable=self.search_var, width=24).pack(side="left", padx=(16, 4))
        tk.Button(toolbar, text="Prev", command=self._search_prev).pack(side="left")
        tk.Button(toolbar, text="Next", command=self._search_next).pack(side="left", padx=(4, 0))
        tk.Button(
            toolbar,
            text="打开日志文件夹",
            command=lambda: open_log_dir(APP_NAME),
        ...
        self.text.tag_config("hit", underline=1, background="#ffe58a")
    ...
    def _search_next(self) -> None:
        ...
        if not idx:
            idx = self.text.search(needle, "1.0", stopindex="end", nocase=True)
            if not idx:
                return
    ...
    def _search_prev(self) -> None:
        ...
        if not needle:
            return
        self.text.tag_remove("hit", "1.0", "end")
        start_index = self._last_search_index
        if start_index == "1.0":
            start_index = "end"
        idx = self.text.search(
            needle,
